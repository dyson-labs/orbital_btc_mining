...
with open(os.path.join(ROOT, "config", "orbits_to_test.json"), "r") as f:
...
@app.route("/orbit_image/<int:idx>")
def orbit_image(idx: int):
    """Return PNG for the selected orbit."""
...
def api_simulate():
    ...
    times, illum = env.illumination_profile(dt=60, n_periods=1)
    T_hist, x, thermal_buf, temp_stats = run_thermal_eclipse_model(
        illumination_profile=(times, illum), plot3d=True, verbose=False
    )

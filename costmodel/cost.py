...
def run_cost_model(solar_fraction, **kwargs):
    ...
    mission_lifetime     = int(kwargs.get("mission_lifetime", 5))
    ...
    btc_price_growth         = float(kwargs.get("btc_price_growth", 10)) / 100   # %/yr to decimal
    network_hashrate_growth  = float(kwargs.get("network_hashrate_growth", 25)) / 100 # %/yr to decimal
    ...
    for year in range(mission_lifetime):
        btc_price_year = btc_price * ((1 + btc_price_growth) ** year)
        net_hash_year = network_hashrate_ehs * ((1 + network_hashrate_growth) ** year)
        btc_price_list.append(btc_price_year)
        net_hash_list.append(net_hash_year)

    avg_btc_price = sum(btc_price_list) / mission_lifetime
    avg_network_hashrate = sum(net_hash_list) / mission_lifetime
